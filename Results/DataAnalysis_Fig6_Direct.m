...
for i = 1:length(mineDurs)
    ...
    for j = 1:length(mineLaunchDates)
        ...
        if ~isempty(data.TotTOF(data.AsteroidUncertainty == 0    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'Direct' & data.LaunchDate == mineLaunchDates(j)  ))
            mineTOFs(j,1) = max(data.TotTOF(data.AsteroidUncertainty == 0    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'Direct' & data.LaunchDate == mineLaunchDates(j)  )); 
        else
            mineTOFs(j,1) = NaN;
        end
        
        if ~isempty(data.TotTOF(data.AsteroidUncertainty == 0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'Direct' & data.LaunchDate == mineLaunchDates(j)  ))
            mineTOFs(j,2) = max(data.TotTOF(data.AsteroidUncertainty == 0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'Direct' & data.LaunchDate == mineLaunchDates(j)  ));  
        else
            mineTOFs(j,2) = NaN;
        end
        
        if ~isempty(data.TotTOF(data.AsteroidUncertainty == -0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'Direct' & data.LaunchDate == mineLaunchDates(j)  ))
            mineTOFs(j,3) = max(data.TotTOF(data.AsteroidUncertainty == -0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'Direct' & data.LaunchDate == mineLaunchDates(j)  ));  
        else
            mineTOFs(j,3) = NaN;
        end
