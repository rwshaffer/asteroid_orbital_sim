...
for i = 10:16
    ...
    for j = 1:length(mineLaunchDates)
        ...
        if ~isempty(data.TotTOF(data.AsteroidUncertainty == 0    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'EGA' & data.LaunchDate == mineLaunchDates(j)  ))
            mineTOFs(j,1) = data.TotTOF(data.AsteroidUncertainty == 0    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'EGA' & data.LaunchDate == mineLaunchDates(j)  ); 
        else
            mineTOFs(j,1) = NaN;
        end
        
        if ~isempty(data.TotTOF(data.AsteroidUncertainty == 0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'EGA' & data.LaunchDate == mineLaunchDates(j)  ))
            mineTOFs(j,2) = data.TotTOF(data.AsteroidUncertainty == 0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'EGA' & data.LaunchDate == mineLaunchDates(j)  );  
        else
            mineTOFs(j,2) = NaN;
        end
        
        if ~isempty(data.TotTOF(data.AsteroidUncertainty == -0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'EGA' & data.LaunchDate == mineLaunchDates(j)  ))
            mineTOFs(j,3) = data.TotTOF(data.AsteroidUncertainty == -0.1    & data.MiningDuration == mineDurs(i) & string(data.OutTrajType) == 'EGA' & data.LaunchDate == mineLaunchDates(j)  );  
        else
            mineTOFs(j,3) = NaN;
        end
