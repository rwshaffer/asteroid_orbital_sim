...
if nargin< 4
    ...
    if nargin<3
        ...
        interceptororbit.raan=-11.26064*pi/180; %rads
        interceptororbit.aop=85.901*pi/180; %rads
        interceptororbit.nu=100.46435*pi/180-interceptororbit.aop-interceptororbit.raan; %rads
        if nargin<2
...
for n=1:length(date)
    ...
    for i=1:length(t)
        ...
        [v1,v2]=GaussProblem(rinterceptorinitial,rtargetfinal,t(i));
        deltavinitial=abs(norm(v1)-norm(vinterceptorinitial));
        deltavfinal=abs(norm(vtargetfinal)-norm(v2));
