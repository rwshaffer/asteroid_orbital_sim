...
%   target.inc - inclination (deg)
%   target.raan - right ascension of the ascending node (deg)
%   target.aop - argument of periapsis (deg)
%   target.nu - true anomaly (deg)
...
if earth_orbit == "eccentric"
    ...
    earth.inc = 0; % inclination above ecliptic (deg)
    earth.raan = 197.91; % right ascension (deg)
    earth.aop = 267.452; % argument of periapsis (deg)
    j2000_mean_long = 100.46435; % Earth's mean longitude at epoch (deg)
    mean_anomaly = mod(j2000_mean_long - (earth.raan + earth.aop),360); % mean anomaly (deg)
    earth.nu = rad2deg * convert_anomalies(mean_anomaly*deg2rad,earth.ecc,'mean'); % true anomaly (deg)
else
    earth.ecc = 0; % eccentricity
    earth.inc = 0; % inclination above ecliptic (deg)
    earth.raan = 0; % right ascension (deg)
    earth.aop = 0; % argument of periapsis (deg)
    j2000_mean_long = 100.46435; % Earth's mean longitude at epoch (deg)
    earth.nu = j2000_mean_long; % true anomaly (deg) w.r.t vernal equinox
end
...
earth.traj = @(nu) earth.p/(1 + earth.ecc*cosd(nu)); % Trajectory equation, input true anomaly (deg). Output rad (km).
...
target.traj = @(nu) target.p/(1 + target.ecc*cosd(nu)); % Trajectory equation, input true anomaly (deg). Output rad (km).
...
earth.nu = KeplerProblem(earth,sun_mu,earth_TOF); % True anomaly (deg) at new epoch
...
target.nu = KeplerProblem(target,sun_mu,target_TOF); % True anomaly (deg) at new epoch
...
target.nu_asc = 360 - target.aop; % true anomaly of ascending node (deg)
target.nu_desc = target.nu_asc + 180; % true anomaly of descending node (deg)
earth.nu_target_asc = target.raan; % Earth's true anomaly at target ascending node (deg)
earth.nu_target_desc = earth.nu_target_asc + 180; % Earth's true anomaly at target descending node (deg)
...
%% STEP 0: Find launch window when Earth crosses target's line of nodes
...
target.nu_launch = KeplerProblem(target,sun_mu,target.wait_TOF); % Target true anomaly at launch time (deg)
...
transfer.phi_1 = 0; % satellite asymptote angle (deg)
...
%% Step 1: Hyperbolic escape from Earth's SOI
...
target.nu = KeplerProblem(target,sun_mu,transfer.TOF); % target true anomaly (deg) when S/C reaches node
target.epoch = target.epoch + transfer.TOF * sec2day; % Update target epoch to match new true anomaly
delta_phi = target.nu_asc - target.nu; % Phase angle (deg) between S/C and target when S/C reaches the target's node
...
fprintf("Note: still need to incorporate maneuvers around Earth to get into inclined parking orbit. \n\n")
end
